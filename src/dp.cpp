...
int findNumOfLongestIncreasingSequence(vector<int>& seqs)
{
    int seq_len, max_ans;
    seq_len=seqs.size();
    vector<int> dp(seq_len),ans(seq_len);
    dp[0]=1; // the length of LIS w/ i as tail
    ans[0]=1;// the number of LIS w/ i as tail
    max_ans=1;
    for(int i=1; i <seq_len; ++i)
    {
        ...
        for(int j=0; j <i; ++j)
        {
            if(seqs[j]<seqs[i])
            {
                if(dp[j]+1>dp[i])//this seq w/ this longer length firstly come out
                {
                    dp[i]=dp[j]+1;
                    ans[i]=ans[j]; // so reset ans
                }
                else if(dp[j]+1==dp[i])
                {
                    ans[i]+=ans[j]; // different seq w/ same length, so accumulate
                }
            }
...
{
    ENTER1("LIS test");
    vector<int> seqs1{1,3,5,4,7};
    vector<int> seqs2{2,2,2,2,2};
    std::cout << findNumOfLongestIncreasingSequence(seqs1) << endl;
    std::cout << findNumOfLongestIncreasingSequence(seqs2) << endl;
}
